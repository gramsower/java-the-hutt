...
function EditBeanForm (props) {
  ...
  function handleEditBeanFormSubmission(event) {
    ...
    props.onEditBean({name: event.target.name.value, price: event.target.price.value, roast: event.target.roast.value, origin: event.target.origin.value, tastingNotes: event.target.tastingNotes.value, id: bean.id});
  }
